module ApplicationHelper
  ICONS = {
    ...
    chart_bar: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>',
    ...
    pencil: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>',
    ...
    x_circle: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>',
    ...
    arrow_left: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>',
    alert_circle: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',
    ...
    content_tag(:nav, class: "breadcrumbs") do
      safe_join(
        items.each_with_index.map do |item, i|
          if i == items.length - 1
            content_tag(:span, item[:label], class: "current")
          else
            separator = content_tag(:span, icon(:chevron_right), class: "separator")
            link = link_to(item[:label], item[:path])
  ...
  def format_currency(amount)
    "Rp #{number_with_delimiter(amount.to_i)}"
  end
  ...
  def time_ago_short(time)
    ...
    if diff < 60
      ...
      "#{(diff / 60).to_i} menit lalu"
    elsif diff < 86400
      "#{(diff / 3600).to_i} jam lalu"
    elsif diff < 604800
      "#{(diff / 86400).to_i} hari lalu"
    else
      time.strftime("%d/%m/%Y")
    end
  end
